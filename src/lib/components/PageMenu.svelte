...
<nav class="relative w-fit" use:clickoutside onclickoutside={() => (open = false)}>
  <label class="flex cursor-pointer items-center gap-2 p-2 transition-colors hover:text-[var(--link-hover-color)]">
  ...
  {#if open}
    <div
      class="absolute right-0 top-full z-10 mt-1.5 w-max overflow-hidden rounded-md border border-[var(--menu-border)] bg-[var(--menu-bg)] shadow-lg"
    >
      <ul>
        {#each links as link}
          <li>
